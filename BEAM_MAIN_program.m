  function amp_list = BEAM_MAIN_program(Young_mat_1,Den_mat_1,Young_mat_2,Den_mat_2,Spring_Pos)
% Young_mat_1 = 210e9;  Den_mat_1 = 7850;
% Young_mat_2 = 210e9;  Den_mat_2 = 7850;
% Spring_Pos = 0; %default = -1
...
%le = 1;
Element_no = 20;
elem_len = [0.05 , 0.05] ;  % (Element 2, Element 1)
Nodes = Element_no + 1;
DOF = Nodes*2;
...
% f(4) = f(4) + M;
if Spring_Pos>=0 
    spring_nodes(1) = Spring_Pos;
    if(Spring_Pos == 0)
        spring_nodes = [7,19,31,39];
    
    elseif(Spring_Pos == 0.1)
            spring_nodes = [7,19,31,39,7,19,31,39];
    end

%Adding Spring mass system 

for i = 1:1:length(spring_nodes)
k_spring = 319280;
m_spring = 0.098;
if(i>=5)
    if(i<7)
...
increment = 1;
Y_val = zeros(DOF-2,1);
Amp_Y_real = zeros(length(init_freq:increment:Final_freq),1);
for f = init_freq:increment:Final_freq
    omeg = 2*pi*f;
    for i = 1:1:DOF-2
    ...
    %disp(int8(((f-init_freq)/increment)+1));
   Amp_Y_real(int16(((f-init_freq)/increment)+1),1) = sqrt(temp_amp_per_mode)/W(Point_Study);
end
